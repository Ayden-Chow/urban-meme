...
{	
  ...
  float capacitence1;
	...
	while(1)
	{
		...
		capacitance= 1.44/((Ra+2*Rb)*F);
    if(!BUT_AVG){
      waitms(50);
      if(!BUT_AVG){
        totcap += capacitance;
        capcount++;
        capacitence=totcap/capcount;
        LED=1;
      }
    }else{
      LED=0;
    }
		if( capacitance*1000000 > 0.01 ) {
			LCDprint("High Cap", 1, 1);
			sprintf(buff,"%g", capacitance*100000);
			strncat(buff, unit, 6);
			LCDprint(buff,2,1);
		}
		
		else {
			LCDprint("Low Cap", 1, 1);
			sprintf(buff,"%g", capacitance*1000000000);
			strncat(buff, unit2, 6);
			LCDprint(buff,2,1);
